...
export const DoctorPatientAccessCenter: React.FC<DoctorPatientAccessCenterProps> = ({
  ...
  const [bioPatientQuery, setBioPatientQuery] = useState<string>("NH-IND-2026-88392014");
  ...
  const handleAppointmentAccess = () => {
    ...
    startSessionAndOpenRecord(
      apt.patientHealthId || "NH-IND-2026-88392014",
      "APPOINTMENT",
  ...
  const handleBiometricAccess = async (targetIdOverride?: string) => {
    const targetHealthId = (targetIdOverride || bioPatientQuery || appointments[0]?.patientHealthId || "NH-IND-2026-88392014").trim();
    if (!targetHealthId) {
  ...
  return (
    <div className="space-y-6">
      {/* HEADER BANNER */}
      <div className="bg-[#13192B] border border-slate-800 rounded-3xl p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-white tracking-tight">Doctor Patient Access Center</h1>
        ...
        <div className="px-4 py-2 bg-[#0D121F] border border-slate-800 rounded-2xl text-xs font-mono text-slate-300 flex items-center space-x-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          { key: "APPOINTMENT", label: "1. Appointment", icon: Calendar, color: "hover:border-blue-500" },
          { key: "PATIENT_HEALTH_ID", label: "2. Patient Health ID", icon: User, color: "hover:border-indigo-500" },
          { key: "PATIENT_ACCESS_CARD", label: "3. Access Card / QR", icon: CreditCard, color: "hover:border-emerald-500" },
          { key: "BIOMETRIC", label: "4. Biometric / Device", icon: Fingerprint, color: "hover:border-purple-500" },
        ].map((m) => {
          const Icon = m.icon;
          ...
          return (
            <button
              ...
              className={`p-4 rounded-2xl border flex flex-col items-center justify-center text-center space-y-2 transition shadow-lg ${
                isSelected
                  ? "bg-purple-600 border-purple-400 text-white shadow-purple-600/30 font-bold"
                  : `bg-[#13192B] border-slate-800 text-slate-400 hover:text-white ${m.color}`
              }`}
            >
        ...
        {selectedMethod === "APPOINTMENT" && (
          <div className="space-y-4">
            <div className="flex items-center space-x-3 border-b border-slate-800 pb-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Access Patient Record via Appointment</h3>
                <p className="text-xs text-slate-400">Select a scheduled appointment to open the complete clinical record.</p>
              </div>
            </div>

            {appointments.length === 0 ? (
              <div className="p-8 text-center text-slate-400 text-xs bg-[#0D121F] rounded-2xl border border-slate-800 space-y-3">
              ...
              <div className="space-y-3">
                    ...
                    <div
                      ...
                      className={`p-4 rounded-2xl border cursor-pointer transition flex items-center justify-between ${
                        selectedAppointmentId === apt.id || (!selectedAppointmentId && appointments[0].id === apt.id)
                          ? "bg-blue-950/80 border-blue-500/80 text-white shadow-lg"
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="px-2 py-0.5 bg-blue-900/60 text-blue-300 text-[10px] font-mono rounded font-bold">
                            Token #{apt.tokenNumber || "101"}
                          </span>
                        </div>
                        <div className="text-xs text-slate-400 font-mono">
                          Health ID: {apt.patientHealthId || "NH-IND-2026-88392014"}
                        </div>
                      ...
                      <CheckCircle2 className={`w-5 h-5 ${selectedAppointmentId === apt.id ? "text-blue-400" : "text-slate-600"}`} />
                    </div>
                  ))}
                </div>
                ...
                <div className="pt-3 flex justify-end">
                  <button
                    ...
                    className="px-6 py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-blue-900/40 transition flex items-center space-x-2"
                  >
        ...
        {selectedMethod === "PATIENT_HEALTH_ID" && (
          <div className="space-y-4">
            <div className="flex items-center space-x-3 border-b border-slate-800 pb-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Access Patient Record via Global Health ID</h3>
                <p className="text-xs text-slate-400">Enter the patient's unique Global Health ID or full registered name.</p>
              </div>
            </div>

            <div className="space-y-3 max-w-lg">
              <div>
                <label className="text-slate-400 text-xs block mb-1">Global Health ID / Patient Name</label>
                <div className="relative">
                  <input
                    type="text"
                    value={healthIdQuery}
                    onChange={(e) => setHealthIdQuery(e.target.value)}
                    placeholder="e.g. NH-IND-2026-88392014 or Ananya Sharma"
                    className="w-full bg-[#0D121F] border border-slate-800 rounded-xl px-4 py-3 text-white text-sm font-mono focus:border-indigo-500 focus:outline-none"
                  />
              ...
              <div className="pt-2 flex justify-end">
                <button
                  ...
                  disabled={isAccessing}
                  className="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-indigo-900/40 transition flex items-center space-x-2"
                >
          ...
          <div className="space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800 pb-3">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">Access Patient Record via Physical Access Card / QR</h3>
                  <p className="text-xs text-slate-400">Scan physical card QR code or enter token identifier manually.</p>
                </div>
              </div>

              <button
                onClick={() => setShowMobileBridgeModal(true)}
                className="px-4 py-2.5 bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 hover:from-purple-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl shadow-lg transition flex items-center justify-center space-x-2 shrink-0 border border-purple-400/30"
              >
                <Smartphone className="w-4 h-4 text-purple-200 animate-bounce" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {/* Camera Scanner */}
              <div className="bg-[#0D121F] border border-slate-800 rounded-2xl p-4 flex flex-col items-center justify-center space-y-3">
                {isCameraActive ? (
                  <div className="relative w-full h-48 bg-black rounded-xl overflow-hidden border border-emerald-500/50 flex items-center justify-center">
                    <video ref={videoRef} className="w-full h-full object-cover" />
                    <canvas ref={canvasRef} className="hidden" />
                    <div className="absolute inset-0 border-2 border-emerald-400 border-dashed animate-pulse pointer-events-none" />
                  </div>
                ) : (
                  <div className="w-full h-48 bg-[#13192B] rounded-xl border border-slate-800 flex flex-col items-center justify-center space-y-2 text-slate-400">
                ...
                <button
                  ...
                  className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl transition flex items-center space-x-2"
                >
              ...
              {/* Manual Token Input */}
              <div className="space-y-3">
                <label className="text-slate-400 text-xs block mb-1">Card Identifier / Secure Token</label>
                <input
                  type="text"
                  value={cardTokenQuery}
                  onChange={(e) => setCardTokenQuery(e.target.value)}
                  placeholder="e.g. CARD-PAT-1001 or NEXUSHEALTH:NH-IND-2026-88392014"
                  className="w-full bg-[#0D121F] border border-slate-800 rounded-xl px-4 py-3 text-white text-sm font-mono focus:border-emerald-500 focus:outline-none"
                />
                ...
                <div className="pt-2 flex justify-end">
                  <button
                    ...
                    disabled={isAccessing}
                    className="px-6 py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl shadow-lg shadow-emerald-900/40 transition flex items-center space-x-2"
                  >
        ...
        {selectedMethod === "BIOMETRIC" && (
          <div className="space-y-4">
            <div className="flex items-center space-x-3 border-b border-slate-800 pb-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Access Patient Record via Biometrics, Face Scan & Phone Lock</h3>
                <p className="text-xs text-slate-400">Scan face using mobile phone camera, phone lock fingerprint, or WebAuthn / Passkey on device.</p>
              </div>
            </div>

            {/* Target Patient Selection Box */}
            <div className="bg-[#090D16] border border-slate-800 rounded-2xl p-4 space-y-3">
              <label className="text-xs font-bold text-purple-300 flex items-center space-x-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                  <span className="text-[11px] text-slate-400 block mb-1">Select Patient from OPD Queue:</span>
                  <select
                    value={bioPatientQuery}
                    onChange={(e) => setBioPatientQuery(e.target.value)}
                    className="w-full bg-[#121829] border border-slate-700 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-purple-500"
                  >
                    <option value="NH-IND-2026-88392014">Ananya Sharma (NH-IND-2026-88392014)</option>
                    <option value="NH-IND-2026-99281045">Rohan Verma (NH-IND-2026-99281045)</option>
                    {appointments.map((apt) => (
                ...
                <div>
                  <span className="text-[11px] text-slate-400 block mb-1">Or Enter Patient Health ID / Name:</span>
                  <input
                    type="text"
                    value={bioPatientQuery}
                    onChange={(e) => setBioPatientQuery(e.target.value)}
                    placeholder="e.g. NH-IND-2026-99281045 or Rohan Verma"
                    className="w-full bg-[#121829] border border-slate-700 rounded-xl px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 font-mono"
                  />
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="bg-[#0D121F] border border-purple-500/30 hover:border-purple-500/70 rounded-2xl p-5 flex flex-col items-center justify-between text-center transition space-y-4">
                <div className="w-12 h-12 rounded-2xl bg-purple-950 border border-purple-500/40 flex items-center justify-center text-purple-300">
                  <Smartphone className="w-6 h-6 animate-pulse" />
                </div>
                <div>
                  <h4 className="font-bold text-white text-sm">📱 Mobile Phone Camera & Biometrics Bridge</h4>
              ...
              <div className="bg-[#0D121F] border border-slate-800 hover:border-slate-700 rounded-2xl p-5 flex flex-col items-center justify-between text-center transition space-y-4">
                ...
                <div>
                  <h4 className="font-bold text-white text-sm">👆 TouchID / Passkey Authenticator</h4>
                  <p className="text-xs text-slate-400 mt-1">
                ...
                {biometricStatus && (
                  <div className="p-2 bg-purple-950/80 border border-purple-500/50 rounded-xl text-[11px] text-purple-200 w-full">
                    {biometricStatus}
                  </div>
                )}
                ...
                <button
                  ...
                  className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-purple-300 font-bold text-xs rounded-xl border border-slate-700 transition flex items-center justify-center space-x-2"
                >
